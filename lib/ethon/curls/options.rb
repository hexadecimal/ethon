  ...
  module Curls
    ...
    module Options
      ...
      def set_option(option, value, handle, type = :easy)
        ...
        when :ffipointer
          func = :ffipointer
          raise Errors::InvalidValue.new(option, value) unless value.nil? || value.is_a?(FFI::Pointer)
        when :curl_slist
          func = :ffipointer
          raise Errors::InvalidValue.new(option, value) unless value.nil? || value.is_a?(FFI::Pointer)
        when :buffer
        ...
        when :callback
          func = :callback
          raise Errors::InvalidValue.new(option, value) unless value.nil? || value.is_a?(Proc)
        when :debug_callback
          func = :debug_callback
          raise Errors::InvalidValue.new(option, value) unless value.nil? || value.is_a?(Proc)
      ...
      # Regarding :enum
      # Two ways to specify values (as opts parameter):
      ...
      #
      # Reagarding :bitmask
      # Three ways to specify values (as opts parameter):
      ...
      def self.option(ftype, name, type, num, opts = nil)
        ...
        when :bitmask
          if opts.is_a? Array
            ...
            opts.each_with_index do |value, i|
                next if value.nil?
                if i == 0
                  hopts[value] = 0
                else
          ...
          opts.each do |key, value|
            if value.is_a? Array
              opts[key] = value.map { |b| opts[b] }.inject :|
            end
          end
      ...
      def self.option_type(type)
        ...
        define_method(OPTION_STRINGS[type]) do |rt|
          return Ethon::Curls::Options.const_get(cname).map { |key, value| [key, value[:opt]] } if rt == :enum
      ...
      option :easy, :progressfunction, :progress_callback, 56 # Obsolete
