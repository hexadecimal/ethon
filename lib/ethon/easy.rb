...
module Ethon
  ...
  # See https://curl.haxx.se/libcurl/c/libcurl-easy.html for more information.
  #
  # @example You can access the libcurl easy interface through this class,
  # every request is based on it. The simplest setup looks like that:
  #
  #   request = Ethon::Easy.new(url: "www.example.com")
  #   request.perform
  #   #=> :ok
  #
  ...
  class Easy
    ...
    #   * :weird_server_reply: The server sent data libcurl couldn't parse.
    #     This error code is used for more than just FTP
    #     and is aliased as CURLE_WEIRD_SERVER_REPLY since 7.51.0.
    ...
    #   * :http2: A problem was detected in the HTTP2 framing layer.
    #     This is somewhat generic and can be one out of several problems,
    #     see the error buffer for details.
    ...
    #   * :peer_failed_verification: The remote server's SSL certificate
    #     or SSH md5 fingerprint was deemed not OK.
    ...
    #   * :no_connection_available: No connection available, the session will be queued
    ...
    def log_inspect
      "EASY #{mirror.log_informations.map { |key, value| "#{key}=#{value}" }.flatten.join(' ')}"
    end
  end
end
