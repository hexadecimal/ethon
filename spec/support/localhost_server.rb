...
#   https://github.com/teamcapybara/capybara/blob/0.3.9/lib/capybara/server.rb
class LocalhostServer
  ...
  private
    ...
    def boot
      ...
      options = { Port: port }
      unless ENV['VERBOSE_SERVER']
        options[:AccessLog] = []
        options[:Logger] = WEBrick::BasicLog.new(StringIO.new)
      end
    ...
    def wait_until(timeout, error_message, &block)
      ...
      while true
        return if yield
        raise TimeoutError.new(error_message) if (Time.now.utc - start_time) > timeout
        sleep(0.05)
      end
    end
end
