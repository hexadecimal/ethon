...
describe Ethon::Easy::ResponseCallbacks do
  ...
  [:on_complete, :on_headers, :on_body, :on_progress].each do |callback_type|
    describe "##{callback_type}" do
      it "responds" do
        expect(easy).to respond_to(callback_type.to_s)
      end

      context "when no block given" do
        it "returns @#{callback_type}" do
          expect(easy.send(callback_type.to_s)).to eq([])
        end
      end
  ...
  describe "#progress" do
    ...
    context "when pretending" do
      before do
        ...
        @ultotal = nil
        @ulnow = nil
        easy.on_progress do |dltotal, dlnow, ultotal, ulnow|
          @dltotal = dltotal
          @dlnow = dlnow
          @ultotal = ultotal
          @ulnow = ulnow
